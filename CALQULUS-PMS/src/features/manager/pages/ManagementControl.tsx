        ...
        <div className="space-y-6">
          {!selectedId ? <Card><CardContent className="py-16 text-center"><p className="font-medium">Select a managed property</p><p className="mt-1 text-sm text-muted-foreground">The configuration below applies to one owner relationship at a time.</p></CardContent></Card> : <>
            <Card><CardHeader><CardTitle>Client profile</CardTitle><CardDescription>Classify the owner without turning an independent manager into an agency.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
              <Field label="Client type"><Select value={draft.client_type} onValueChange={(v) => set("client_type", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["individual","family","company","institution","staff_quarters","nonprofit","other"].map((v) => <SelectItem key={v} value={v}>{v.replace(/_/g, " ")}</SelectItem>)}</SelectContent></Select></Field>
            ...
            <Card><CardHeader><CardTitle>Owner portal & reporting</CardTitle><CardDescription>Give each owner the right level of visibility and a predictable reporting relationship.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 rounded-xl border border-border p-4"><p className="text-sm font-semibold">Owner visibility</p><p className="mt-1 text-xs text-muted-foreground">Choose what the owner can see in their existing Landlord portal for this managed property.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{["property","units","occupancy","tenants","maintenance","vendors","documents","contracts","leases","collections","financials","distributions"].map((key) => <Toggle key={key} label={key.replace(/_/g, " ")} checked={draft.owner_visibility[key] ?? false} onChange={(v) => setMap("owner_visibility", key, v)} />)}</div></div>
              <Field label="Reporting frequency"><Select value={draft.reporting_frequency} onValueChange={(v) => set("reporting_frequency", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["none","exception_only","weekly","monthly","quarterly"].map((v) => <SelectItem key={v} value={v}>{v.replace(/_/g, " ")}</SelectItem>)}</SelectContent></Select></Field>
              <Field label="Reporting delivery"><Select value={draft.reporting_delivery} onValueChange={(v) => set("reporting_delivery", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["portal","email","portal_and_email"].map((v) => <SelectItem key={v} value={v}>{v.replace(/_/g, " + ")}</SelectItem>)}</SelectContent></Select></Field>
              <div className="sm:col-span-2 rounded-xl border border-border p-4"><p className="text-sm font-semibold">Report sections</p><p className="mt-1 text-xs text-muted-foreground">Control the content of scheduled owner reports without creating another reporting engine.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{["occupancy","tenant_service","maintenance","vendors","compliance","financial_summary","collections","distributions","documents"].map((key) => <Toggle key={key} label={key.replace(/_/g, " ")} checked={draft.report_sections[key] ?? false} onChange={(v) => setMap("report_sections", key, v)} />)}</div></div>
