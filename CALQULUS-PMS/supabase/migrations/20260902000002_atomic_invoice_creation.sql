-- CALQULUS Phase 3: Atomic invoice creation and idempotent generation.
-- One canonical write path for monthly/automated tenant invoices.

ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS generation_key text;

CREATE UNIQUE INDEX IF NOT EXISTS invoices_generation_key_unique
...
BEGIN
  ...
  -- Idempotency: insert-or-return-existing. ON CONFLICT waits for a concurrent
  -- transaction, then DO NOTHING lets us fetch the committed winner.
  ...
  IF jsonb_array_length(COALESCE(p_line_items, '[]'::jsonb)) > 0
     AND round(v_line_total, 2) <> round(p_amount, 2) THEN
    RAISE EXCEPTION 'Invoice line items do not equal invoice total' USING ERRCODE = '22023';
  END IF;

  INSERT INTO public.invoices (
    invoice_number, lease_id, tenant_id, unit_id, property_id, manager_id,
    amount, original_amount, balance_due, paid_amount, description, due_date,
    status, generation_key
  ) VALUES (
    '', p_lease_id, p_tenant_id, p_unit_id, p_property_id, p_manager_id,
    round(p_amount, 2), round(p_amount, 2), round(p_amount, 2), 0,
    p_description, p_due_date, 'pending', p_generation_key
  )
  ...
  IF v_invoice.id IS NULL THEN
    ...
    IF v_existing.id IS NULL THEN
      RAISE EXCEPTION 'Invoice idempotency conflict could not be resolved' USING ERRCODE = '40001';
    END IF;
  ...
  IF v_lease.id IS NULL THEN
    RAISE EXCEPTION 'Lease does not belong to the supplied tenant, property, unit and manager' USING ERRCODE = '42501';
  END IF;

  SELECT manager_id INTO v_tenant_manager
  FROM public.tenants
  ...
  IF p_property_id IS NOT NULL THEN
    SELECT manager_id INTO v_property_manager
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_line_items, '[]'::jsonb)) LOOP
    INSERT INTO public.invoice_line_items (
      invoice_id, unit_charge_id, charge_type, charge_label,
      quantity, unit_price, amount, is_manual
    ) VALUES (
      v_invoice.id,
      NULLIF(v_item->>'unit_charge_id', '')::uuid,
      COALESCE(v_item->>'charge_type', 'other'),
      COALESCE(v_item->>'charge_label', 'Charge'),
      COALESCE((v_item->>'quantity')::numeric, 1),
      COALESCE((v_item->>'unit_price')::numeric, 0),
      COALESCE((v_item->>'amount')::numeric, 0),
      COALESCE((v_item->>'is_manual')::boolean, false)
    );
  END LOOP;
...
REVOKE ALL ON FUNCTION public.create_invoice_atomic(text, uuid, uuid, uuid, uuid, uuid, numeric, text, date, jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_invoice_atomic(text, uuid, uuid, uuid, uuid, uuid, numeric, text, date, jsonb)
  TO service_role;
