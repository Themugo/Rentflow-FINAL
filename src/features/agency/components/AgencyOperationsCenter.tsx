...
import { BriefcaseBusiness, FileCog, LockKeyhole, Plus, Save, Settings2, ShieldCheck, Users, WalletCards } from "lucide-react";
...
export default function AgencyOperationsCenter() {
  ...
  return (
    <AgencyLayout title="Agency Operations" description="Your Agency defines client contracts, permissions, billing rules and operating controls. CALQULUS enforces the rules you set." actions={<Badge variant="outline" className="gap-1.5"><BriefcaseBusiness className="h-3.5 w-3.5 text-primary" /> {canAdmin ? "Agency Admin" : "Read only"}</Badge>}>
      {!canAdmin ? <Alert className="mb-5"><LockKeyhole className="h-4 w-4" /><AlertTitle>Read-only access</AlertTitle><AlertDescription>An Agency owner or administrator must grant you configuration permissions before you can change these controls.</AlertDescription></Alert> : null}
